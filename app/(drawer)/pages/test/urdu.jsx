...
const UrduTestScreen = () => {
  ...
  const panResponder = PanResponder.create({
    ...
    onPanResponderGrant: (e, gestureState) => {
      const x = (gestureState.x0 - offset.x) / SCALE_FACTOR;
      const y = (gestureState.y0 - offset.y) / SCALE_FACTOR;
      setCurrentPath([{ x, y }]);
    },
    onPanResponderMove: (e, gestureState) => {
      const x = (gestureState.moveX - offset.x) / SCALE_FACTOR;
      const y = (gestureState.moveY - offset.y) / SCALE_FACTOR;
  ...
  return (
    <View style={styles.container}>
      <ImageBackground
        source={require("../../../../assets/images/urdubg.jpg")}
      ...
      <View style={styles.controls}>
        <Text style={styles.title}>اردو ٹیسٹ</Text>
        <TouchableOpacity style={styles.clearButton} onPress={clearDrawing}>
          <Text style={styles.clearButtonText}>صاف کریں</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
